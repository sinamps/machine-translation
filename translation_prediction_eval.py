...
from mbert_transformation_learner_linear import MyModel
...
def feed_model(base_model, model, data_loader, dataset, l1_texts, l2_texts):
    rand_display_index = random.sample([i for i in range(0, len(dataset))], 100)
    out_display_file = open('display_examples.txt', 'w', encoding='utf-8')
    outputs_all = []
    ...
    for index in range(len(dataset)):
        ...
        dists.clear()
        if index in rand_display_index:
            out_display_file.write("Input Sentence L1: <<< ")
            out_display_file.write(l1_texts[index])
            out_display_file.write(' >>>\n')
            for k, v in enumerate(rand_index):
                out_display_file.write("Candidate Sentence " + str(k) + ": <<< ")
                out_display_file.write(l2_texts[v])
                out_display_file.write(' >>>\n')
            out_display_file.write("Candidate Sentence 4 (Correct L2): <<< ")
            out_display_file.write(l2_texts[index])
            out_display_file.write(' >>>\n')
            out_display_file.write("Chosen Candidate: ")
            out_display_file.write(str(m))
            out_display_file.write('\n\n')
...
if __name__ == '__main__':
    ...
    test_outputs = feed_model(base_model, model, test_data_loader, test_dataset, test_l1_texts, test_l2_texts)
